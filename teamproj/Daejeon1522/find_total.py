...
def process_csv(file_path, district_name, save_path):
    ...
    result = data[['location_id', 'SUM_NRG_USQNT', 'SUM_NRG_TOE_USQNT', 'SUM_GRGS_DSAMT']]

    # 슬라이스된 result에서 NaN을 0으로 설정할 때, .loc를 사용
    result.loc[(abs(result['SUM_NRG_USQNT']) < 1e-7) | (abs(result['SUM_NRG_USQNT']) > 1e+10), 'SUM_NRG_USQNT'] = 0

    result.loc[(abs(result['SUM_NRG_TOE_USQNT']) < 1e-7) | (abs(result['SUM_NRG_TOE_USQNT']) > 1e+10), 'SUM_NRG_TOE_USQNT'] = 0

    result.loc[(abs(result['SUM_GRGS_DSAMT']) < 1e-5) | (abs(result['SUM_GRGS_DSAMT']) > 1e+10), 'SUM_GRGS_DSAMT'] = 0

    result.columns = ['location_id', 'sum_nrg_usqnt', 'sum_nrg_toe_usqnt', 'sum_grgs_dsamt']

    # 결과를 새로운 CSV 파일로 저장 (저장 경로 포함)
    output_file = os.path.join(save_path, csv_file.split('.')[0] + '_total.csv')
