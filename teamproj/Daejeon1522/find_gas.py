...
def process_csv(file_path, district_name, save_path):
    ...
    result = data[['location_id', 'CTY_GAS_USQNT', 'CTY_GAS_TOE_USQNT', 'CTY_GAS_GRGS_DSAMT']]

    # 슬라이스된 result에서 NaN을 0으로 설정할 때, .loc를 사용
    result.loc[:, 'CTY_GAS_USQNT'] = result['CTY_GAS_USQNT'].fillna(0)
    result.loc[:, 'CTY_GAS_TOE_USQNT'] = result['CTY_GAS_TOE_USQNT'].fillna(0)
    result.loc[:, 'CTY_GAS_GRGS_DSAMT'] = result['CTY_GAS_GRGS_DSAMT'].fillna(0)

    result.columns = ['location_id', 'cty_gas_usqnt', 'cty_gas_toe_usqnt', 'cty_gas_grgs_dsamt']

    # 결과를 새로운 CSV 파일로 저장 (저장 경로 포함)
    output_file = os.path.join(save_path, csv_file.split('.')[0] + '_gas.csv')
