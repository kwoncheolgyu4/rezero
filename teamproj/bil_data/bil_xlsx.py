...
if final_combined_data:
    combined_df = pd.concat(final_combined_data, ignore_index=True)
    ...
    # 결과를 CSV로 저장
    output_file = os.path.join(output_folder, "all_districts_combined.csv")
    grouped_df.to_csv(output_file, index=False, encoding="utf-8-sig")
    print(f"모든 구 데이터를 합친 결과 저장 완료: {output_file}")